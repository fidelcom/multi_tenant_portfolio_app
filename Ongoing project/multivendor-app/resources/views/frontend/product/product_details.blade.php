                                        ...
                                        <div class="product-thumb-area">
                                            ...
                                            <div class="thumb-wrapper one filterd-items figure">
                                                <div class="product-thumb zoom" onmousemove="zoom(event)"
                                                     style="background-image: url({{ asset($product->product_thumbnail) }})"><img
                                                        src="{{ asset($product->product_thumbnail) }}" alt="product-thumb">
                                                </div>
                                            </div>
                                            @foreach($product->product_multiImage as $key=>$img)
                                            <div class="thumb-wrapper two{{$key}} filterd-items hide">
                                                <div class="product-thumb zoom" onmousemove="zoom(event)"
                                                     style="background-image: url({{ asset($img->name) }})"><img
                                                        src="{{ asset($img->name) }}" alt="product-thumb">
                                                </div>
                                            </div>
                                            @endforeach
                                            <div class="product-thumb-filter-group">
                                                <div class="thumb-filter filter-btn active" data-show=".one"><img
                                                        src="{{ asset($product->product_thumbnail) }}" alt="product-thumb-filter"></div>
                                                @foreach($product->product_multiImage as $key=>$img)
                                                <div class="thumb-filter filter-btn" data-show=".two{{ $key }}"><img
                                                        src="{{ asset($img->name) }}" alt="product-thumb-filter"></div>
                                                @endforeach
                                            </div>
                                        </div>
                                        <div class="contents">
                                            <div class="product-status">
                                                <span class="product-catagory">{{ $product->product_category->name }}</span>
                                            ...
                                            <h2 class="product-title" id="dpname">{{ $product->name }}</h2>

                                            @if($product->discount)
                                                <span class="product-price mb--15 d-block" style="color: #DC2626; font-weight: 600;"> ${{ number_format($product->discount, 2) }}<span class="old-price ml--15">${{ number_format($product->price, 2) }}</span></span>
                                            @else
                                                <span class="product-price mb--15 d-block" style="color: #DC2626; font-weight: 600;"> ${{ number_format($product->price, 2) }}</span>
                                            @endif
                                            <p class="mt--20 mb--20">
                                                {!! $product->short_desc !!}
                                            </p>
                                            <div class="d-flex justify-content-between">
                                                ...
                                                <div class="" id="colorArea">
                                                    ...
                                                    <select name="dcolor" class="select" id="dcolor">
                                            ...
                                            <div class="product-bottom-action">
                                                <div class="cart-edits">
                                                    <div class="quantity-edit action-item">
                                                        ...
                                                        <input type="text" class="input" id="dqty" value="01"/>
                                                        <button class="button plus">+<i class="fal fa-plus plus"></i></button>
                                                    </div>
                                                </div>
                                                <a type="submit" class="rts-btn btn-primary radious-sm with-icon" onclick="addToCartDetails()">
                                                    <div class="btn-text">
                                                        Add To Cart
                                                    </div>
                                            ...
                                            <div class="product-uniques">
                                                <span class="sku product-unipue mb--10"><span style="font-weight: 400; margin-right: 10px;">Product Code: </span> {{ $product->product_code }}</span>
                                                <span class="catagorys product-unipue mb--10"><span style="font-weight: 400; margin-right: 10px;">Category: </span> {{ $product->product_category->name }}</span>
                                                <span class="tags product-unipue mb--10"><span style="font-weight: 400; margin-right: 10px;">Tags: </span> {{ $product->tags }}</span>
                                                ...
                                                <span class="tags product-unipue mb--10"><span style="font-weight: 400; margin-right: 10px;">Subcategory: </span> {{ $product->product_subcategory->name }}</span>
                                            </div>
                                        ...
                                        <div class="single-shopping-card-one">
                                            ...
                                            <div class="image-and-action-area-wrapper">
                                                <a href="#" class="thumbnail-preview">
                                                    ...
                                                    <img src="{{ asset($related_product->product_thumbnail) }}" alt="{{ $related_product->name }}">
                                                </a>
                                            ...
                                            <div class="body-content">
                                                ...
                                                <a href="#">
                                                    <h4 class="title">{{ $related_product->name }}</h4>
                                                </a>
                                                <span class="availability">500g Pack</span>
                                                <div class="price-area">
                                                    @if($related_product->discount)
                                                        <span class="current">${{ number_format($related_product->discount, 2) }}</span>
                                                        <div class="previous">${{ number_format($related_product->price, 2 ) }}</div>
                                                    @else
                                                        <span class="current">${{ number_format($related_product->price, 2) }}</span>
                                                    @endif
                                                </div>
